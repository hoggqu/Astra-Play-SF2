...
function write_observation(snapshot)
 ...
 local busy=job~=nil or bot~=nil or advance~=nil or loadwatch~=nil or savewatch~=nil or (entry_busy and entry_busy()) or (play_busy and play_busy()) or (train_busy and train_busy()) or false
 ...
 f:write(string.format('{\n  "rom":"sf2",\n  "screenshot":"%03d.png",\n  "emulated_seconds":%.6f,\n  "paused":%s,\n  "throttled":%s,\n  "throttle_rate":%.3f,\n  "speed_factor":%d,\n  "controller_busy":%s,\n  "difficulty_bits":%d,\n  "difficulty_mirror":%d,\n  "effective_difficulty":%d,\n  "ai_rank":%d,\n  "ai_parameter_a":%d,\n  "ai_parameter_b":%d,\n  "ai_index":%d,\n  "session_gate":%s,\n  "timer_bcd":%d,\n  "timer_seconds":%d,\n  "p1":%s,\n  "p2":%s\n}\n',
  ...
  tostring(busy),d.difficulty_bits,d.difficulty_mirror,d.effective_difficulty,d.ai_rank,d.ai_parameter_a,d.ai_parameter_b,d.ai_index,astra_entry and astra_entry.json() or 'null',timer,seconds,player(0,a),player(1,b)))
 f:close()
