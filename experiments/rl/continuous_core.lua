-- Experimental driving only; reuse unchanged native result/lifecycle state machine.
...
function Core:drive(s,effects)
 if self.rl_round~=self.round then effects.input=self:rl_prime(s);return effects end
