"""Replay a retained non-TIME KO timer tail and verify natural next-round inputs."""
import argparse
import importlib
import json
...
def run(source, failure, dataset, output, snapshot_frame=1667, block=64):
    ...
    try:
        ...
        assert len(rounds)==1 and rounds[0]['outcome']=='win'
        assert not original['rounds']
        settled=rounds[0]
        assert settled['score']==[1,0]
        assert settled['recognition']=='rl-native-nontime-ko-timer-tail-v11'
        assert settled['frame']==1667 and settled['stable_award_frames']==360
        assert settled['stop']['timer']==60 and settled['settled']['timer']==59
        tail=settled['ko_award']['timer_tail']
        assert tail['frame']==1307 and tail['previous']['frame']==1306
        ...
        result.update(status='complete',frames=frames,decisions=len(transitions),
                      native_result=settled,next_round_decisions=len(after),
...
if __name__=='__main__':
    ...
    p.add_argument('--snapshot-frame',type=int,default=1667)
