-- Experimental result-only adapter v10. Stage as rl_settlement.lua in NEW runs.
...
return function(Core)
 ...
 Core.rl_settlement_adapter='native-pip-ko-late-ko-time-and-confirmed-draw-v10'
   ...
   if side then
    ...
    if evidence.award_frame and self.frame-evidence.award_frame>=360
     ...
     local row={round=self.round,outcome=side==1 and 'win' or 'loss',opening=self.round_opening,
      ...
      recognition=late and (late.zero_hp_edge and 'rl-native-zero-hp-time-ko-v10' or 'rl-native-unlatched-time-ko-v7') or 'rl-native-nontime-pip-ko-v6',ko_award=evidence,late_ko=late,
