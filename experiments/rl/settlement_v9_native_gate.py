"""Replay a retained adjacent TIME-draw failure and verify natural next-round inputs."""
import argparse
import importlib
import json
...
def run(source, failure, dataset, output, snapshot_frame=3500, block=64):
    ...
    try:
        ...
        rounds=reference['rounds'];assert len(rounds)==1 and rounds[0]['outcome']=='draw'
        assert rounds[0]['score']==original['score']
        assert rounds[0]['recognition']=='rl-native-equal-time-adjacent-ko-v9'
        assert rounds[0]['confirmation_frame']==original['frame']
        assert rounds[0]['settled_frame']>=original['terminal_frame']+360
        ...
        result.update(status='complete',frames=frames,decisions=len(transitions),
                      ...
                      compared_original_frames=len(original['trace']),next_round_first_macro_frames=12,
...
if __name__=='__main__':
    ...
    p.add_argument('--snapshot-frame',type=int,default=3500)
