...
本次候选已通过独立原生门禁：全部 16 动作、768 帧，两种训练启动方式与连续
参考的逐帧状态、事件和实际端口一致，22 次普通技末帧释放检查通过。随后
4 worker 的 2048 决策真实 PPO 检查完成，参数发生更新，最大 Lua/Torch
旧策略 log-probability 差为 `2.431e-6`，小于原来的 `2e-5` 门槛。
该检查模型单独保留；第一批完整 20 次使用迁移后的原模型 `f26bb8be52583209`，
不是检查阶段更新后的模型。完整成绩以最终结果为准。
