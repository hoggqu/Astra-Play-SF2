"""Compare screen-perception sampling with independent native-time forced execution."""
import argparse
import importlib
import json
...
def run(source, dataset, output):
    ...
    manifest, pins = source_pins(source.parent, source.name)
    OBSERVATIONS = manifest['observations']
    if OBSERVATIONS != 4516:
        raise ValueError('This gate requires the screen-perception interface')
    sample = select_train(dataset, (3,), 1)[0]
    output.mkdir(parents=True, exist_ok=False)
    result = dict(schema='astra.rl-perception-native-gate.v2', status='running', training_only=True,
    ...
    try:
        ...
        for section, ids in enumerate((list(range(43)), list(range(43, 85)))):
            ...
            for role in ('batch', 'manual_reset', 'native'):
                ...
                if role == 'native':
                    ...
                    if role == 'manual_reset':
                        ...
                        if obs.shape != (OBSERVATIONS,): raise RuntimeError('Reset observation dimension mismatch')
                    offset = 0
            ...
            for role in ('batch', 'manual_reset'):
                ...
                if actual.shape != (len(plan), OBSERVATIONS): raise RuntimeError('Observation dimension mismatch')
                np.testing.assert_array_equal(actual, expected)
            result['cases'].append(dict(section=section, actions=ids, frames=len(reference),
                decisions=len(plan), status='pass', observations=OBSERVATIONS,
