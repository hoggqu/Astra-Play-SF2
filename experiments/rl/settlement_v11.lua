-- Experimental result-only adapter v11. Stage as rl_settlement.lua in NEW runs.
...
return function(Core)
 ...
 Core.rl_settlement_adapter='native-pip-ko-late-ko-time-and-confirmed-draw-v11'
 ...
 function Core:tick(s)
  ...
  if self.phase=='settling' and self.round_stop.timer==0 and s.timer==0
   ...
   local side=ko(b.hp) and health(a.hp) and a.hp>0 and 1
    or ko(a.hp) and health(b.hp) and b.hp>0 and 2 or nil
   ...
   if side and oldscore and score_ok and p.timer==0
    ...
    and ((p.p1.hp>0 and p.p2.hp>0) or zero_edge)
    ...
    self.rl_late_ko={state=s,previous=previous,frame=self.frame,side=side,zero_hp_edge=zero_edge or nil}
   end
  end
   ...
   if side then
    ...
    if evidence.award_frame and self.frame-evidence.award_frame>=360
     ...
     local row={round=self.round,outcome=side==1 and 'win' or 'loss',opening=self.round_opening,
      ...
      recognition=late and (late.zero_hp_edge and 'rl-native-zero-hp-time-ko-v10' or 'rl-native-unlatched-time-ko-v7') or (evidence.timer_tail and 'rl-native-nontime-ko-timer-tail-v11' or 'rl-native-nontime-pip-ko-v6'),ko_award=evidence,late_ko=late,
