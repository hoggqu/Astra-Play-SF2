"""Build a separate perception128 source with batch-boundary input-latch parity."""
import argparse
import json
from pathlib import Path
import shutil
import tempfile
from .perception128_timing_identity import (INPUT_NAMES,PACKAGE,SCHEMA,PROTOCOL,TIMING_PROTOCOL,
...
def build(source,output):
    ...
    shutil.copytree(source,output/'timing-parent',ignore=ignore)
    ...
    (output/'timing-inputs').mkdir()
    for n,b in inputs.items():(output/'timing-inputs'/n).write_bytes(b)
    ...
    frozen={p.relative_to(output).as_posix():digest(p.read_bytes())
            for name in ('timing-parent','timing-inputs','src')
            for p in (output/name).rglob('*') if p.is_file()}
    frozen['launch.py']=digest(launcher.encode())
    manifest=dict(parent)
    manifest.update(schema=SCHEMA,package=PACKAGE,settlement_protocol=PROTOCOL,input_timing_protocol=TIMING_PROTOCOL,
                    ...
                    scope='Restore the prior held input before same-time RPC resume; unchanged native Core, policy, action waveforms, reward and optimizer.')
    (output/'build.json').write_text(json.dumps(manifest,indent=2)+'\n')
    validate_build(manifest,package)
    if validate_parent(source)[1]!=captured:raise RuntimeError('Parent changed during input timing build')
    return manifest


def bootstrap(output):
    from .perception128_late_ko_builder import bootstrap as make_parent
    with tempfile.TemporaryDirectory(prefix='astra-input-timing-') as folder:
...
def main():
    ...
    print(json.dumps({k:result[k] for k in ('schema','package','settlement_protocol','input_timing_protocol','parent_build_sha256')}))
