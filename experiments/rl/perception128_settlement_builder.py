"""Build a separate perception128 source with generic native non-TIME KO proof."""
import argparse
import json
from pathlib import Path
import shutil
import tempfile
from .perception128_settlement_identity import (INPUT_NAMES,PACKAGE,SCHEMA,PROTOCOL,
...
def build(source,output):
    ...
    manifest.update(schema=SCHEMA,package=PACKAGE,settlement_protocol=PROTOCOL,
                    ...
                    scope='Result-only non-TIME KO chronology; all policy/input/reward/optimizer bytes unchanged.')
...
def bootstrap(output):
    from .perception128_builder import bootstrap as make_parent
