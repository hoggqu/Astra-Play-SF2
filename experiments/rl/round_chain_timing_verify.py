"""Full-match native parity after corrected batch-resume timing; new outcomes allowed."""
import argparse
import importlib
import json
...
def run(source,training,original,dataset,output,chain_evidence=None):
    ...
    result={'schema':'astra.rl-round-chain-timing-parity.v1','status':'running','training_only':True,
    ...
    try:
            ...
            try:
                ...
                if role=='chain':
                    ...
                    while offset<len(actions):
                        ...
                        if (output/'chain/training/rl-chain-match-00001.json').exists():break
        ...
        for n in range(2,len(captured['rounds'])+1):
            after=[r for r in reference['decisions'] if r['round']==n]
            if len(after)<2 or not after[0]['reset_history'] or after[1]['frame']-after[0]['frame']!=12:
                raise RuntimeError('Natural next-round initial action/reset cadence missing')
        result.update(status='complete',native_frames=len(trace),decisions=len(reference['decisions']),
            outcomes=[r['outcome'] for r in captured['rounds']],
            initial_loads=reference['initial_loads'],in_play_pauses=reference['in_play_pauses'],
            trajectory_scope='new corrected-input trajectory; does not reproduce old double-KO path',
