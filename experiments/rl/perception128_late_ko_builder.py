"""Build a separate perception128 source with native post-clock-zero KO proof."""
import argparse
import json
from pathlib import Path
import shutil
import tempfile
from .perception128_late_ko_identity import (INPUT_NAMES,PACKAGE,SCHEMA,PROTOCOL,
...
def build(source,output):
    ...
    manifest.update(schema=SCHEMA,package=PACKAGE,settlement_protocol=PROTOCOL,
                    ...
                    scope='Result-only unlatched post-clock-zero KO chronology; all policy/input/reward/optimizer bytes unchanged.')
...
def bootstrap(output):
    from .perception128_settlement_builder import bootstrap as make_parent
