...
import tempfile
import unittest
from .perception128_settlement_builder import bootstrap
from .perception128_settlement_identity import validate_build
...
class SettlementBuildTests(unittest.TestCase):
    def test_exact_derivation_parent_model_and_relocation(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            parent=root/'settlement-parent/astra_sf2_rl_perception128'
            ...
            for name in ('initialize.py','support.py','batch_train.py'):
                expected=(parent/name).read_bytes().replace(b'from .perception128_identity import',b'from .perception128_settlement_identity import')
            ...
            try:
                ...
                metadata=importlib.import_module(manifest['package']+'.perception128_settlement_identity').training_metadata(package)
            ...
            code="""import json
from pathlib import Path
from astra_sf2_rl_perception128_ko.perception128_settlement_identity import validate_build
r=Path.cwd();validate_build(json.loads((r/'build.json').read_text()),r/'astra_sf2_rl_perception128_ko')
"""
