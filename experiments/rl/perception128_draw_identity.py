"""Result-only zero-HP late-KO revision, bound to an immutable perception128 parent."""
import importlib
...
SCHEMA='astra.rl-screen-perception128-settlement.v10'
PROTOCOL='native-pip-ko-late-ko-time-and-confirmed-draw-v10'
TIMING_PROTOCOL='restore-held-input-before-rpc-resume-v1'
INPUT_NAMES=('perception128_draw_builder.py','perception128_draw_identity.py','settlement_v10.lua')
...
def validate_parent(root):
    ...
    importlib.import_module(name+'.perception128_timing_identity').validate_build(manifest,package)
    return manifest,{n:(package/n).read_bytes() for n in manifest['derived_sha256']}


def derive(captured,inputs):
    out=dict(captured)
    out['settlement.lua']=inputs['settlement_v10.lua']
    ...
    for name in ('initialize.py','support.py','batch_train.py'):
        ...
        out[name]=out[name].replace(b'from .perception128_timing_identity import',b'from .perception128_draw_identity import')
    return out
...
def validate_build(manifest,package):
    ...
    expected={'schema':SCHEMA,'package':PACKAGE,'settlement_protocol':PROTOCOL,
              ...
              'decision_frames':12,'input_timing_protocol':TIMING_PROTOCOL,'history':4,'frame_features':1129,
...
def training_metadata(package):
    ...
    return {'perception_build_sha256':digest((package.parent/'build.json').read_bytes()),
            ...
            'combat_policy':'Shared PPO128; unchanged actions/observations/reward/input timing; strictly observed zero-HP late KO plus prior settlement rules'}
