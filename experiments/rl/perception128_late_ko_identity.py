"""Result-only native late-KO revision, bound to an immutable perception128 parent."""
import importlib
...
PACKAGE='astra_sf2_rl_perception128_late_ko'
SCHEMA='astra.rl-screen-perception128-settlement.v7'
PROTOCOL='native-pip-ko-late-ko-time-and-confirmed-draw-v7'
INPUT_NAMES=('perception128_late_ko_builder.py','perception128_late_ko_identity.py','settlement_v7.lua')
...
def validate_parent(root):
    ...
    importlib.import_module(name+'.perception128_settlement_identity').validate_build(manifest,package)
    return manifest,{n:(package/n).read_bytes() for n in manifest['derived_sha256']}


def derive(captured,inputs):
    out=dict(captured)
    out['settlement.lua']=inputs['settlement_v7.lua']
    out['perception128_late_ko_identity.py']=inputs['perception128_late_ko_identity.py']
    for name in ('initialize.py','support.py','batch_train.py'):
        out[name]=out[name].replace(b'from .perception128_settlement_identity import',b'from .perception128_late_ko_identity import')
    return out
...
def training_metadata(package):
    ...
    return {'perception_build_sha256':digest((package.parent/'build.json').read_bytes()),
            ...
            'combat_policy':'Shared PPO128; unchanged input/actions/reward; native late-KO result revision only'}
