-- Experimental result-only adapter v7 candidate. Stage as rl_settlement.lua in NEW runs.
...
return function(Core)
 ...
 Core.rl_settlement_adapter='native-pip-ko-late-ko-time-and-confirmed-draw-v7'
 ...
 function Core:tick(s)
  ...
  if self.phase=='settling' and self.round_stop.timer==0 and s.timer==0
   ...
   if side and oldscore and score_ok and p.timer==0
    ...
    self.rl_late_ko={state=s,previous=previous,frame=self.frame,side=side}
   end
  end
   ...
   if side then
    ...
    if evidence.award_frame and self.frame-evidence.award_frame>=360
     ...
     local row={round=self.round,outcome=side==1 and 'win' or 'loss',opening=self.round_opening,
      ...
      recognition=late and 'rl-native-unlatched-time-ko-v7' or 'rl-native-nontime-pip-ko-v6',ko_award=evidence,late_ko=late,
